...
4. **Know which server you are on** — every SQL file has its own connection, shown in the status bar (`$(database) analytics-prod`) and in the results panel title. Click the status bar item to bind the file to a different connection; a file with no binding of its own follows the workspace default and is marked `(default)`. The first run on an unbound file asks once and remembers. DDL documents opened from the explorer inherit the connection they came from, and *Use This Connection for the Active File* on a connection's context menu binds from the other direction. Bindings persist per workspace.
